...
# Generate particles to be simulated throughout using a uniform distribution
...
for n in ns:
    ...
    initial_2D_positions = np.random.uniform(0,1,(n,2)) * box_size
    initial_3D_positions = np.random.uniform(0,1,(n,3)) * box_size
    for i, initial_2D_position in enumerate(initial_2D_positions):
        # Particles have positive properties
        initial_2D_particles.append(Particle(initial_2D_position, np.random.uniform(0,max_property)))
        initial_3D_particles.append(Particle(initial_3D_positions[i], np.random.uniform(0,max_property)))
...
for i, n in enumerate(ns):
    for j, p in enumerate(ps):
        ...
        fmm_direct_diff_result = util.calc_difference_results(fmm_particles, direct_2D_particles_collection[i], plotting=False)
        
        fmm_direct_diff_result.sim_time = sim_time
        log_fmm_direct_diff_result = util.calc_log_relative_error_results(fmm_particles, direct_2D_particles_collection[i], plotting=False)
